...
function createFilterDict() {
	...
	if (document.getElementById("acousticCheck").checked) {
		dict["target_acousticness"] =
			document.getElementById("acousticInput").value;
	}
	...
	if (document.getElementById("instrumentalCheck").checked) {
		dict["target_intrumentalness"] =
			document.getElementById("instrumentalInput").value;
	}
	...
	if (document.getElementById("timeSigCheck").checked) {
		dict["target_time_signature"] =
			document.getElementById("timeSigInput").value;
	}
...
const processNumTracksChange = debounce(getPlaylist, 275);
...
function tunableTrackStatusChange(element) {
	if (element.checked) {
		element.parentElement.parentElement.lastElementChild.firstElementChild.style.background =
			"green";
		getPlaylist();
	} else {
		element.parentElement.parentElement.lastElementChild.firstElementChild.style.background =
